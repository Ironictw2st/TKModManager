...
# Usage: powershell -File scripts\ui.ps1 -X 100 -Y 96 [-Text "se_"] [-Keys "{ENTER}"] [-Wheel -5]
param([int]$X, [int]$Y, [string]$Text = "", [string]$Keys = "", [string]$Title = "TK Mod Manager", [string]$Process = "", [int]$Wheel = 0)
Add-Type @"
using System;
...
if ($Wheel -ne 0) {
  # Scroll instead of clicking: positive = up, in notches.
  for ($i = 0; $i -lt [Math]::Abs($Wheel); $i++) { [UiW]::mouse_event(0x0800, 0, 0, $(if ($Wheel -gt 0) { [uint32]120 } else { [uint32]4294967176 }), [UIntPtr]::Zero); Start-Sleep -Milliseconds 30 }
} else {
